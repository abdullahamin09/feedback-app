const FeedbackSidebar = ({ filterCategory, setFilterCategory, roadmapCounts, openRoadmap, openAddFeedback }) => {
  return (
    <div className="flex flex-col gap-6 w-full max-w-[255px] font-sans">
      ...
      <div className="bg-white p-6 rounded-xl flex flex-wrap gap-x-2 gap-y-3 shadow-sm">
        {['All', 'UI', 'UX', 'Enhancement', 'Bug', 'Feature'].map((category) => (
          <button 
            key={category}
            className={`px-4 py-1.5 rounded-lg text-[13px] font-semibold transition-colors ${filterCategory === category ? 'bg-[#4661E6] text-white' : 'bg-[#F2F4FF] text-[#4661E6] hover:bg-[#CFD7FF] '}`}
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={openRoadmap} className="text-[#4661E6] underline text-[13px] font-semibold hover:text-[#8397F8] cursor-pointer">
            View
          </button>
        </div>
        ...
        <ul className="space-y-3">
          ...
          <li className="flex justify-between items-center text-[#647196]">
            ...
            <span className="font-bold">{roadmapCounts.inProgress}</span>
          </li>
          {/* Live Item */}
          <li className="flex justify-between items-center text-[#647196]">
            <div className="flex items-center gap-4">
        ...
        className="w-full mt-6 bg-[#AD1FEA] hover:bg-[#C75AF6] text-white font-bold py-3 px-4 rounded-xl text-[13px] transition-colors">
          + Add Feedback
        </button>
      </div>
